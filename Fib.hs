...
fibRecBN n 
    | invalidBigNumber n = error "Please introduce a valid Big Number"
    | head n < 0 = error "negative numbers are not part of the fibonacci sequence"    
    | otherwise = fibRecBN' n [0] [1]
...
fibListaBN n 
    | invalidBigNumber n = error "Please introduce a valid Big Number"
    | head n < 0 = error "negative numbers are not part of the fibonacci sequence"  
    | otherwise = last (fibListaBN' n 1 [[0],[1]])
